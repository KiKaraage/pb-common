...
## renovate-automerge.yml — token must be generated by calling workflow

`reusable-renovate-automerge.yml` falls back to `github.token` when no `secrets.token` is passed. `github-actions[bot]` is not in the `main-review-required-with-renovate-bypass` ruleset bypass list — only mergeraptor is. Without a token the reusable cannot squash-merge Renovate PRs into main.

**Fix already in place:** `renovate-automerge.yml` has a `get-token` job that generates the mergeraptor token and passes it as `secrets.token` to the reusable.
