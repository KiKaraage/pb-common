---
name: bluefin-build
description: Bluefin build, validation, and PR workflow — use when working in projectbluefin/bluefin, bluefin-lts, common, or dakota repos. Agent-direct model — no castrojo fork.
---
...
Load with: `cat ~/src/skills/bluefin-build/SKILL.md`
...
- Adding or removing packages only — use `cat ~/src/skills/bluefin-packages/SKILL.md`
- Diagnosing a GitHub Actions failure — use `cat ~/src/skills/bluefin-ci/SKILL.md`
- Working in the LTS repo — also load `cat ~/src/skills/bluefin-lts/SKILL.md`
- Building or promoting ISOs — use `cat ~/src/skills/bluefin-iso/SKILL.md`
...
All 5 projectbluefin repos (bluefin, bluefin-lts, common, dakota, knuckle) share unified labels. Start every session by checking for active blockers:
...
**Branch targets by repo:**
| Repo | Target branch |
|---|---|
| `bluefin`, `bluefin-lts`, `dakota` | `testing` — never `main` |
| `common` | `main` |

**Never use `castrojo` fork for bluefin/bluefin-lts/common/dakota.** Those forks are gone.

## CODEOWNERS & Contributor Roles

Two roles exist across `bluefin`, `bluefin-lts`, `common`, and `dakota`:

| Role | Permission | Can approve |
|---|---|---|
| **Maintainers** | write/admin | All paths |
| **Triagers** | triage (per-repo collaborator) | `docs/**` and `*.md` only |

### Triage section is centrally managed in `common`

The triage block in every repo's `.github/CODEOWNERS` is owned by `projectbluefin/common`. **Never edit it directly in bluefin, bluefin-lts, or dakota.** It is marked with sentinels:
...
To add/remove a triager:
1. Edit the sentinel block in `common/.github/CODEOWNERS` → commit to `main`
2. The `sync-codeowners.yml` workflow auto-pushes the change to the other repos
3. Also grant per-repo triage collaborator access: `gh api repos/projectbluefin/REPO/collaborators/USERNAME --method PUT --field permission=triage`
...
Full governance reference: `projectbluefin/common` → `docs/skills/governance.md`
...
1. Always validate before committing: `bash ~/src/skills/bluefin-build/scripts/validate.sh`
...
bash ~/src/skills/bluefin-build/scripts/validate.sh

# Manual build (bluefin only — bluefin-dx image is a dead SKU, do not build it)
